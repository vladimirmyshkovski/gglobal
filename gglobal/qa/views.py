...
from gglobal.qa.models import (UserQAProfile, Question, Answer, AnswerVote,
                       QuestionVote, AnswerComment, QuestionComment, TaggedWhatever, MyCustomTag)
...
class TagAutocomplete(autocomplete.Select2QuerySetView):
    def get_result_label(self, item):
        return '<a style="text-decoration: none;" title="%s">%s</a>' % (item.description, item.name,)

    def get_queryset(self):
        # Don't forget to filter out results depending on the visitor !
        if not self.request.user.is_authenticated():
            return MyCustomTag.objects.none()

        qs = MyCustomTag.objects.all()
        print(qs)
...
class QuestionIndexView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        items = TaggedWhatever.objects.filter(content_type=question_contenttype)
        context['tags'] = MyCustomTag.objects.filter(
            qa_taggedwhatever_items__in=items).order_by('-id').distinct()[:10]
...
class QualificationQuestionIndexView(QuestionIndexView):
    template_name = 'base/qualification.html'
...
class QuestionDetailView(HitCountDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        get_question_by_pk = Question.objects.get(pk=self.kwargs['pk'])
        context['similar_questions'] = Question.objects.filter(title__icontains=get_question_by_pk.title).exclude(id=self.kwargs['pk'])
        context['similar_tags'] = TaggedWhatever.objects.filter(object_id=self.kwargs['pk'])
...
def profile(request, user_id):
    ...
    all_tags = MyCustomTag.objects.all()


    users_questions = all_questions.filter(user_id=user_id)
    users_answers = all_answers.filter(question__user_id=user_id)
    #users_tags = Question.objects.filter(id__in=users_questions)

    whatever_users_tags = TaggedWhatever.objects.filter(object_id__in=users_questions)

    users_tags = all_tags.filter(id__in=whatever_users_tags)
